  ...
  const channelChartData = [
    {
      ...
      data: investmentList
        ...
        .map((item) => ({ name: item.label.replace(/Meta \(|\)/gi, "").slice(0, 12), value: item.value })),
    },
    {
      label: "Google",
      color: "#4285F4",
      data: investmentList
        ...
        .map((item) => ({ name: item.label.replace(/Google Ads/gi, "Google").slice(0, 12), value: item.value })),
    },
    {
      label: "Mídia Offline",
      color: "#EAB308",
            ...
            <div id="ads-performance-card" className="bg-[#0B010C] rounded-2xl shadow-xl flex flex-col p-4 text-white border border-[#2b0c2e] lg:col-span-5 h-auto lg:h-full lg:min-h-0 justify-between gap-4 lg:gap-0">
              ...
              <div className="flex items-center justify-between gap-2 mb-3 shrink-0">
                <div>
                  ...
                  <p className="text-[10px] text-white/50 mt-0.5 leading-tight">Resumo de investimentos e resultados</p>
                </div>
                  ...
                  {(["Meta", "Google", "Offline"] as const).map((subtab) => (
                    <button
                      ...
                      className={`px-2 py-1 text-[9px] font-bold rounded transition-all ${
                        darkCardTab === subtab ? "bg-[#A60069] text-white shadow" : "text-white/60 hover:text-white"
                      }`}
                    >
              ...
              <div className="grid grid-cols-2 sm:grid-cols-4 gap-1.5 mt-3 shrink-0">
                ...
                <div className="bg-white/5 rounded-xl border border-white/10 p-2 flex flex-col justify-between">
                  <span className="text-[8px] uppercase tracking-wider font-extrabold text-white/50 leading-none">Investimento</span>
                ...
                <div className="bg-white/5 rounded-xl border border-white/10 p-2 flex flex-col justify-between max-h-[46px]">
                  <span className="text-[8px] uppercase tracking-wider font-extrabold text-white/50 leading-none">Taxa de agendamento</span>
