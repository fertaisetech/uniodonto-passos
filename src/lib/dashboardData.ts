...
const buildMonthlyDocument = (month: MonthKey): MonthlyDashboardDocument => {
  const investmentTotal = calculateInvestmentTotal(baseInvestments);
  const salesCurrent = 86;
  const cacCurrent = Number((investmentTotal / salesCurrent).toFixed(2));
  const summary: SummaryData = {
    beneficiaries: makeMetric(10289, 1.0, 1.05),
    additions: makeMetric(42, 1.0, 1.1),
    cancellations: makeMetric(18, 1.0, 1.0),
    investment: makeMetric(investmentTotal, 0.98, 1.1),
    roi: makeMetric(819.69, 0.9, 1.15),
    leads: makeMetric(420, 0.94, 1.1),
    appointments: makeMetric(108, 0.92, 1.08),
    sales: makeMetric(salesCurrent, 0.93, 1.12),
    cac: makeMetric(cacCurrent, 0.91, 0.95),
    nps: makeMetric(80, 0.95, 1.05),
  };
  ...
  const beneficiariesData: BeneficiariesData = {
    ...
    distribution: [
      { plan: "Premium", count: 4200 },
      { plan: "Standard", count: 3889 },
      { plan: "Basic", count: 2200 },
    ],
  };

  const funnelData: FunnelDataPoint[] = [
    { stage: "Leads", count: summary.leads.current },
    { stage: "Contatos", count: 300 },
...
export const subscribeMonthlyDashboard = (
  ...
  const unsubscribeRemote = onSnapshot(
    ...
    (error) => {
      handler(null, error instanceof Error ? error : new Error("Falha ao ler o dashboard mensal no Firestore."));
    }
  );
