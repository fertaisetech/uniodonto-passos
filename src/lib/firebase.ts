...
export const resolveRoleFromEmail = (email: string): "Administrador" | "Operador" => {
...
export const saveUserProfile = async (profile: AppUserProfile) => {
  ...
  try {
    await setDoc(userProfileRef(profile.uid), nextProfile, { merge: true });
  } catch {
...
export const loadTeamMembers = async (): Promise<TeamMemberRecord[]> => {
  ...
  return snap.docs.map((document) => document.data() as TeamMemberRecord);
};
...
export const observeTeamMembers = (
  ...
  return onSnapshot(
    ...
    (snap) => {
      onChange(snap.docs.map((document) => document.data() as TeamMemberRecord));
    },
    (error) => {
      if (onError) {
...
export const saveTeamMember = async (member: TeamMemberRecord) => {
  ...
  await setDoc(doc(TEAM_MEMBERS_COLLECTION, member.id), nextMember, { merge: true });
};
